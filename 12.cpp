...
struct grid{
    ...
    vector<char> operator[](size_t y) {return matrix[y];}
    const vector<char> operator[](size_t y) const {return matrix[y];}
    inline char operator()(size_t x, size_t y){
        return matrix[y][x];
    }
    
    // decltype(auto) operator[](this auto& self, size_t idx) 
    // { 
    //     return self.mVector[idx]; 
    // }
    // constexpr char& operator[](size_t x, size_t y){
    //     return rows[y][x];
    // }
